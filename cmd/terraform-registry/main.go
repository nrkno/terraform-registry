...
func init() {
	...
	flag.StringVar(&logFormatStr, "log-format", "text", "Formats: json, text")
...
func main() {
	...
	for _, item := range strings.Split(envJSONFiles, ",") {
		...
		if err := setEnvironmentFromJSONFile(prefix, filename); err != nil {
			logger.Panic("failed to load environment from file(s)",
	...
	if !reg.IsAuthDisabled {
		...
		if err != nil {
			logger.Panic("failed to load auth tokens",
				zap.Errors("err", []error{err}),
			)
		}
		if len(tokens) == 0 {
	...
	case "github":
		gitHubRegistry(reg)
	default:
		logger.Panic("invalid store type", zap.String("selected", storeType))
	}
...
func gitHubRegistry(reg *registry.Registry) {
	if gitHubToken == "" {
		logger.Panic("missing environment var GITHUB_TOKEN")
	}
	if gitHubOwnerFilter == "" && gitHubTopicFilter == "" {
		logger.Panic("at least one of -github-owner-filter and -github-topic-filter must be set")
	}
